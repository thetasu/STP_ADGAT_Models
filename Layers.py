...
class Graph_Attention(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha ,concat=True, residual=False):
        ...
        # ACL18 Dataset
        # self.W_static = nn.Parameter(torch.zeros(85,85).type(torch.FloatTensor), requires_grad=True)
        # KDD17 Dataset
        self.W_static = nn.Parameter(torch.zeros(50,50).type(torch.FloatTensor), requires_grad=True)
    ...
    def get_relation(self, input_r, relation_static = None):
        ...
        if not isinstance(self.coef_revise,torch.Tensor):
            # ADGAT Dataset 198 nodes
            # self.coef_revise = torch.zeros(198, 198, device = input_r.device) + 1.0 - torch.eye(198, 198,device = input_r.device)
            # ACL18 Dataset 85 nodes
            # self.coef_revise = torch.zeros(85, 85, device = input_r.device) + 1.0 - torch.eye(85, 85, device = input_r.device)
            # KDD17 Dataset 50 nodes
            self.coef_revise = torch.zeros(50, 50, device = input_r.device) + 1.0 - torch.eye(50, 50, device = input_r.device)
