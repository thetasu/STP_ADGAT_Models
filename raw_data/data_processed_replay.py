...
# for aastgcn only needs 700 days
stocks = stocks[stocks['date'] >= '2011/02/08']
# for ADGAT which needs 700 + rnn_length days
...
Xs = np.reshape(Xs,[700,198,5])
Ys = np.reshape(Ys,[700,198])
# directly reshape after choose date for ADGAT
...
with open('latest_x_numerical_aastgcn.pkl','wb') as f:
    pickle.dump(Xs,f)
    f.close()
print('x_numerical save success')
print('9************************************************9')

with open('latest_y_numerical_aastgcn.pkl','wb') as f:
    pickle.dump(Ys,f)
    f.close()
